...
import { scheduleConfig, setConfig } from "./api"
import { genScheduleConfigSig, genSetConfigSig } from "./signature"
...
describe("API - Config", function () {
  ...
  describe("setConfig", function () {
    it("set address config successfully", async function () {
      ...
      // Update
      let ts = 1
      await setConfig(contract, grvt, ts, ts, ConfigID.ADMIN_RECOVERY_ADDRESS, bytes32(wallet()))
    })

    it("can set uint config successfully after a preflight", async function () {
    ...
    it("fails if value was not preflighted", async function () {
      const config = getConfigArray(
