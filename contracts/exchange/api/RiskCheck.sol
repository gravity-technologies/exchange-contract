...
contract RiskCheck is BaseContract, ConfigContract {
  ...
  function _getSubMaintenanceMargin(SubAccount storage subAccount) internal view returns (uint64) {
    ...
    MarginTierBI[MAX_M_MARGIN_TIERS][] memory mmConfigs = _getAllMarginTierBI();
    ...
    for (uint i = 0; i < numPerps; i++) {
      ...
      uint mmConfigIdx = _getMMConfigCurrencyIndex(underlyingCurrency);
      BI memory margin = _getMaintenanceMargin(sizeBI, mmConfigs[mmConfigIdx]);
      BI memory charge = margin.mul(_requireMarkPriceBI(id));
