...
task("deploy-l2-new-target", "Deploy new target on L2")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const governanceCalls = [
            {
                ...
                data: new ethers.utils.Interface(["function approve(address,uint256)"]).encodeFunctionData("approve", [l1SharedBridge, ethers.constants.MaxUint256]),
                value: 0,
            },
        ...
        const {
            scheduleTxReceipt,
            executeTxReceipt
        } = await scheduleAndExecuteGovernanceOp(
            governance,
            l1GovernanceAdmin,
            operation,
        );
