...
export const deployContractUpgradable = async (
  ...
  const deploymentFee = await deployer.estimateDeployFee(contract, constructorArguments || [])
  log(`Estimated deployment cost: ${ethers.formatEther(deploymentFee)} ETH`)

  // Check if the wallet has enough balance
  await verifyEnoughBalance(zkWallet, deploymentFee)

  const a = await hre.zkUpgrades.deployProxy(deployer.zkWallet, contract, [42], { initializer: "initialize" });
  ...
  log(`\n"${contract.contractName}" was successfully deployed:`)
