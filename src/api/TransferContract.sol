...
abstract contract TransferContract is BaseTradeContract {
  ...
  function deposit(
    ...
    sub.balance += int128(uint128(numTokens));
  }
  ...
  function transfer(
    ...
    _perpFunding(fromSub);
    // numTokens are upcasted from uint64 -> int128, which is safe
    int128 numTokensInt128 = int128(uint128(numTokens));
    require(fromSub.balance >= numTokensInt128, "insufficient balance");
    _perpFunding(toSub);
    ...
    // Update balance
    fromSub.balance -= numTokensInt128;
    toSub.balance += numTokensInt128;
  }
}
