...
function _verifyTrade(Trade calldata trade) pure {
  Order calldata takerOrder = trade.takerOrder;
  _verifyOrder(takerOrder, false);

  uint numMakerOrders = trade.makerOrders.length;
  require(numMakerOrders > 0, "empty maker orders");

  uint numLegs = trade.makerOrders[0].makerOrder.legs.length;
  uint64[_MAX_NUM_LEGS] memory qtyPerLeg;
  for (uint i = 0; i < numMakerOrders; i++) {
    Order calldata order = trade.makerOrders[i].makerOrder;
    _verifyOrder(order, true);

    require(order.legs.length == numLegs, "inconsistent num legs");
    for (uint j = 0; j < numLegs; j++) {
      qtyPerLeg[j] += order.legs[j].contractSize;
    }
  }

  for (uint i = 0; i < numLegs; i++) {
    require(qtyPerLeg[i] <= takerOrder.legs[i].contractSize, "invalid qtyPerLeg");
  }
}
